...
public class ResponseReader extends SendRequest {
    ...
    public static String findValueForAttributeFromResponse(JSONObject jsonObject, String attributeName){
            ...
            if(jsonObject.get(key) instanceof JSONArray){
                ...
                for(int i=0;i<jsonArray.length();i++){
                    if (jsonArray.get(i) instanceof JSONObject) {
                        findValueForAttributeFromResponse((JSONObject) jsonArray.get(i), attributeName);
    ...
    public static String getValueForAttributeFromResponse(String attributeName){
        JSONObject jsonObject = new JSONObject(SendRequest.getResponse().getBody().asString());
        return findValueForAttributeFromResponse(jsonObject, attributeName);
    }
}
